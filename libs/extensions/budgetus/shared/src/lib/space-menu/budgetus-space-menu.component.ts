...
import { IListGroup, IBudgetusSpaceDbo } from '@sneat/extension-budgetus-contract';
import { builtInListGroups } from '../pages/lists/built-in-lists';

// budgetus-specific side menu rendered in the space "menu" outlet. Unlike the
...
// budgetus-app), this shows only what template has: a space selector (to switch
// spaces, like sneat-app) and the selected space's lists.
@Component({
  selector: 'budgetus-space-menu',
  templateUrl: './budgetus-space-menu.component.html',
  imports: [
    TitleCasePipe,
    RouterLink,
  ...
  providers: [
    { provide: ClassName, useValue: 'BudgetusSpaceMenuComponent' },
    SpaceComponentBaseParams,
  ],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class BudgetusSpaceMenuComponent extends SpaceBaseComponent {
  ...
  protected override onSpaceDboChanged(): void {
    ...
    const groups: IListGroup[] = this.space
      ...
      : [];
    const dbo = this.space?.dbo as unknown as IBudgetusSpaceDbo | undefined;
