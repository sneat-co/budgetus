...
import { BudgetusCoreServicesModule } from '../../services';
...
import { BudgetusComponentBaseParams } from '../../budgetus-component-base-params';
import { IBudgetusAppStateService } from '../../services';
...
@Component({
  selector: 'budgetus-list',
  templateUrl: './list-page.component.html',
  imports: [
    BudgetusCoreServicesModule,
    NgOptimizedImage,
...
export class ListPageComponent extends BaseListPage implements AfterViewInit {
  ...
  private readonly budgetusAppStateService = inject(IBudgetusAppStateService);
  ...
  constructor() {
    const params = inject(BudgetusComponentBaseParams);

    super(params);
    const budgetusAppStateService = this.budgetusAppStateService;
    this.preloader.markAsPreloaded('list');
    if (location.pathname.includes('/lists')) {
      // TODO: document why & how it is possible
      return;
    }
    budgetusAppStateService.changed.subscribe((appState) => {
  ...
  public clickShowWatchedMovies(): void {
    this.budgetusAppStateService.setShowWatched(this.isHideWatched());
  }
  ...
  // 	if (this.appService.appId === 'budgetus') {
