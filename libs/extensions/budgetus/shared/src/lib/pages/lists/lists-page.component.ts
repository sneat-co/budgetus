...
export class ListsPageComponent extends SpaceBaseComponent {
  private readonly params = inject(BudgetusComponentBaseParams);
  ...
  constructor() {
    ...
    this.budgetusAppStateService.changed.subscribe((appState) => {
  ...
  public clickGroup(group: IListGroup): void {
    ...
    this.budgetusAppStateService.setGroupCollapsed(
      group.title,
    ...
    try {
      ...
      if (this.space) {
        if (this.reordered) {
          ...
          const budgetusDbo = this.space.dbo as unknown as
            | IBudgetusSpaceDbo
            | undefined;
          this.updateListsFromSpace(budgetusDbo?.listGroups); // persisted lists
        }
      } else {
        this.listGroups = [];
      }
