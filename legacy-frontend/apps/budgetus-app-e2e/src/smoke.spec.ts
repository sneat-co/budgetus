...
test('app boots and redirects unauthenticated user to login', async ({
  ...
  await expect(page.locator('budgetus-root')).toBeAttached();
...
test('space-scoped budget route loads without crashing', async ({ page }) => {
  const pageErrors: string[] = [];
  page.on('pageerror', (e) => pageErrors.push(String(e)));

  // Lazy-loads the budgetus space shell + budget route. Unauthenticated, this
  // redirects to login; the assertion is that the app handles it without throwing.
  await page.goto('/space/family/smoke-test-space/budget');
