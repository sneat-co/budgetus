...
export const appRoutes: Route[] = [
  {
    ...
    pathMatch: 'full',
    loadComponent: () =>
      import('./home/budgetus-home-page.component').then(
        (m) => m.BudgetusHomePageComponent,
      ),
    canActivate: [AuthGuard],
    data: { authGuardPipe: () => redirectToLoginIfNotSignedIn },
  },
  {
    // Space-scoped routes host the budgetus pages, mirroring sneat-app's
    // space/:spaceType/:spaceID mount point.
    path: 'space/:spaceType/:spaceID',
    loadChildren: () =>
      import('./space/budgetus-space.routes').then((m) => m.budgetusSpaceRoutes),
  },
  {
